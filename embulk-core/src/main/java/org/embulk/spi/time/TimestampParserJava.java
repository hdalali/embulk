...
class TimestampParserJava extends TimestampParser {
    private TimestampParserJava(final String pattern,
                                final Locale locale,
                                final ZoneOffset defaultZoneOffset) {
        this.pattern = pattern;
        this.defaultZoneOffset = defaultZoneOffset;
        this.formatter = DateTimeFormatter.ofPattern(pattern, locale).withResolverStyle(ResolverStyle.STRICT);
    }

    static TimestampParserJava of(final String pattern, final ZoneOffset defaultZoneOffset) {
        return new TimestampParserJava(pattern, Locale.ENGLISH, defaultZoneOffset);
    }
    ...
    private OffsetDateTime buildOffsetDateTime(final TemporalAccessor given) throws TimestampParseException {
        // Non-offset time zone IDs are intentionally rejected by TimestampParserJava.
